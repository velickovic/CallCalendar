...
class Editorial extends Module
{
	...
	public function hookDisplayHome($params)
	{
		...
		$editorial = new EditorialClass((int)$editorial->id, $this->context->language->id);
 		...
 		$i = 0;
 		$file_list = array();
  		$image_path = _PS_ROOT_DIR_.'/themes/'._THEME_NAME_.'/img/home';
  		
		// get a random image
 		$images = array();
    	if ( $img_dir = @opendir($image_path) ) {
    	...
    	$this->smarty->assign(array(
    			'number_of_active_research_projects' => Project::getNumberOfProjects($this->context->language->id, Context::getContext()->shop->id, true),
    			'profile_name' => Context::getContext()->shop->name,
				'editorial' => $editorial,
				...
				'image_path' => _THEME_IMG_DIR_ . 'home/' . $images[$num]				
			));
